...
class Outdoor:
    vertices = [
        [-1, -0.5, -0.01],  # Frente inferior esquerda
        [1, -0.5, -0.01],   # Frente inferior direita
        [1, 0.5, -0.01],    # Frente superior direita
        [-1, 0.5, -0.01],   # Frente superior esquerda
        [-1, -0.5, 0.01],   # Trás inferior esquerda
        [1, -0.5, 0.01],    # Trás inferior direita
        [1, 0.5, 0.01],     # Trás superior direita
        [-1, 0.5, 0.01],    # Trás superior esquerda
    ]
    ...
    def draw(self, x, y, z, tamanho):
        ...
        for i, face in enumerate(self.facesC):
            for j, vertex in enumerate(face):
                glVertex3fv(self.verticesC[vertex])  # Define a posição do vértice
        glEnd()
